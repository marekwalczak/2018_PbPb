...
// ***** FIT BY MAREK ****** //
   ...
   float tot_inc = (count_data - count_gg - count_tailAlice) / (count_incoh/all_incoh);
   cout << endl << "For the pT range from " << from << " to " << to << ":" << endl;
   cout << "Data:   " << count_data << endl;
   cout << "gg:     " << count_gg << endl;
   cout << "Alice: " << count_tailAlice << endl;
   cout << "Remaining : " << (count_data - count_gg - count_tailAlice) << " would be incoh events" << endl; 
...
//  histo_MC_IncohUps->Scale( incoh_fit_MC / all_incoh );
  ...
  double gg_data_fit = all_data - ups_from_fit - all_tailAlice;
  double i1_data_fit = (count_data - count_tailAlice) / fract_incoh;
  double c1_data_fit = ups_from_fit - i1_data_fit;

  cout << endl;
  cout << "number of fitted gg:	" << gg_data_fit << endl;
  cout << "number of fitted i1:	" << i1_data_fit << endl;
  cout << "number of fitted c1:	" << c1_data_fit << endl;
  cout << endl;

  histo_MC_gg->Scale( gg_data_fit / all_gg );
  histo_MC_IncohUps->Scale( i1_data_fit / all_incoh );
  histo_MC_CohUps->Scale( c1_data_fit / all_coh );
